			...
			static bool Prefix(ChatController __instance)
			{
				...
				if (AmongUsClient.Instance.GameState != InnerNet.InnerNetClient.GameStates.Started)
				{
					...
					else if (text.ToLower().StartsWith("/gm"))
					{
						...
						{
							__instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, ModTranslation.GetString("ChatText", 1));
						}
						handled = true;
					}
				}
				...
				if (AmongUsClient.Instance.NetworkMode == NetworkModes.FreePlay)
				{
					...
					else if (text.ToLower().StartsWith("/color "))
					{
						...
						{
							__instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, ModTranslation.GetString("ChatText", 2));
						}
						col = Math.Clamp(col, 0, Palette.PlayerColors.Length - 1);
						CachedPlayer.LocalPlayer.PlayerControl.SetColor(col);
						__instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, ModTranslation.GetString("ChatText", 3)); ;
					}
				}
