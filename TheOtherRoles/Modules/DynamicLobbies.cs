				...
				if (AmongUsClient.Instance.GameState != InnerNet.InnerNetClient.GameStates.Started)
				{
					...
					{ // Unfortunately server holds this - need to do more trickery
						...
						{ // checking both just cause
							...
							{
								__instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, ModTranslation.GetString("LobbyText", 1));
							}
							else
							{
								...
								if (LobbyLimit != GameOptionsManager.Instance.currentNormalGameOptions.MaxPlayers)
								{
									...
									__instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, string.Format(ModTranslation.GetString("LobbyText", 2), LobbyLimit));
								}
								else
								{
									__instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, string.Format(ModTranslation.GetString("LobbyText", 3), LobbyLimit));
								}
							}
						}
					}
