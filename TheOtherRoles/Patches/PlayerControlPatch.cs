...
namespace TheOtherRoles.Patches
{
	...
	public static class PlayerControlFixedUpdatePatch
	{
		...
		public static void plagueDoctorUpdate()
		{
			...
			if (PlagueDoctor.plagueDoctor != null && (CachedPlayer.LocalPlayer.PlayerControl == PlagueDoctor.plagueDoctor || CachedPlayer.LocalPlayer.PlayerControl.Data.IsDead))
			{
				...
				string text = $"[{ModTranslation.GetString("Game-PlagueDoctor", 2)}]\n";
				...
				foreach (PlayerControl p in CachedPlayer.AllPlayers)
				{
					...
					{
						text += Helpers.cs(Color.red, ModTranslation.GetString("Game-PlagueDoctor", 1));
					}
					else
					{
					...
					foreach (PlayerControl target in CachedPlayer.AllPlayers)
					{
						...
						foreach (PlayerControl source in PlagueDoctor.infected.Values.ToList())
						{
							...
							if (distance <= PlagueDoctor.infectDistance && !anythingBetween)
							{
								...
								// Notify progress to other clients
		...
		public static void updatePlayerInfo()
		{
			...
			foreach (PlayerControl p in CachedPlayer.AllPlayers)
			{
				...
				if (snitchFlag || (Lawyer.lawyerKnowsRole && CachedPlayer.LocalPlayer.PlayerControl == Lawyer.lawyer && p == Lawyer.target) || p == CachedPlayer.LocalPlayer.PlayerControl || CachedPlayer.LocalPlayer.Data.IsDead || local == Poucher.poucher && Poucher.killed.Any(x => x.PlayerId == p.PlayerId))
				{
					...
					if (p == CachedPlayer.LocalPlayer.PlayerControl)
					{
						...
						if (HudManager.Instance.TaskPanel != null)
						{
							...
							tabText.SetText(string.Format(ModTranslation.GetString("Text", 23), taskInfo));
						}
		...
		private static void snitchTextUpdate()
		{
			...
			if (numberOfTasks <= Snitch.taskCountForReveal && (forImpTeam || forKillerTeam || forEvilTeam || forNeutraTeam || isDead))
			{
				...
				{
					Snitch.text = UnityEngine.Object.Instantiate(FastDestroyableSingleton<HudManager>.Instance.KillButton.cooldownTimerText, FastDestroyableSingleton<HudManager>.Instance.transform);
					...
					Snitch.text.gameObject.SetActive(!MeetingHud.Instance);
				}
				else if (!snitchIsDead)
				{
					Snitch.text.text = string.Format(ModTranslation.GetString("Text", 24), playerCompleted, playerTotal);
					if (snitchIsDead) Snitch.text.text = ModTranslation.GetString("Text", 25);
				}
		...
		static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo target)
		{
			...
			if (isMedicReport || isDetectiveReport)
			{
				...
				if (deadPlayer != null && deadPlayer.killerIfExisting != null)
				{
					...
					if (isMedicReport)
					{
						msg = string.Format(ModTranslation.GetString("Text", 26), Math.Round(timeSinceDeath / 1000));
					}
					else if (isDetectiveReport)
					{
						if (timeSinceDeath < Detective.reportNameDuration * 1000)
						{
							msg = string.Format(ModTranslation.GetString("Text", 27), deadPlayer.killerIfExisting.Data.PlayerName);
						}
						else if (timeSinceDeath < Detective.reportColorDuration * 1000)
						{
							var typeOfColor = Helpers.isLighterColor(deadPlayer.killerIfExisting) ? ModTranslation.GetString("Text", 30) : ModTranslation.GetString("Text", 31);
							msg = string.Format(ModTranslation.GetString("Text", 28), typeOfColor);
						}
						else
						{
							msg = ModTranslation.GetString("Text", 29);
						}
					}
	...
	public static class MurderPlayerPatch
	{
		...
		public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
		{
			...
			{
				Helpers.showFlash(new Color(42f / 255f, 187f / 255f, 245f / 255f), message: ModTranslation.GetString("Text", 32));
			}
	...
	public static class PlayerPhysicsFixedUpdate
	{
		public static void Postfix(PlayerPhysics __instance)
		{
			...
			if (Swooper.swooper != null && Swooper.swooper == CachedPlayer.LocalPlayer.PlayerControl && Swooper.isInvisable) __instance.body.velocity *= GameOptionsManager.Instance.currentNormalGameOptions.PlayerSpeedMod + Swooper.swoopSpeed;
        }
    }
