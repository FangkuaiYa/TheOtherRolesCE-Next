        ...
        public class GameStartManagerStartPatch
        {
            public static void Postfix(GameStartManager __instance)
            {
                ...
                if (!Helpers.isCustomServer())
                {
                    ...
                    var onClick = __instance.HostPrivateButton.OnClick = new UnityEngine.UI.Button.ButtonClickedEvent();
                    HudManager.Instance.Notifier.AddDisconnectMessage(ModTranslation.GetString("Text", 46));
            ...
            public static void Postfix(GameStartManager __instance)
            {
                ...
                foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients.ToArray())
                {
                    ...
                    else if (!playerVersions.ContainsKey(client.Id))
                    {
                        ...
                        message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} {ModTranslation.GetString("GameStartText", 1)}\n</color>";
                    }
                    else
                    {
                        ...
                        if (diff > 0)
                        {
                            message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} {ModTranslation.GetString("GameStartText", 2)} (v{playerVersions[client.Id].version.ToString()})\n</color>";
                            versionMismatch = true;
                        }
                        else if (diff < 0)
                        {
                            message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} {ModTranslation.GetString("GameStartText", 3)} (v{playerVersions[client.Id].version.ToString()})\n</color>";
                            versionMismatch = true;
                        }
                        else if (!PV.GuidMatches())
                        { // version presumably matches, check if Guid matches
                            message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} {ModTranslation.GetString("GameStartText", 4)} v{playerVersions[client.Id].version.ToString()} <size=30%>({PV.guid.ToString()})</size>\n</color>";
                            versionMismatch = true;
                        }
                    }
                }
                ...
                else
                {
                    if (!playerVersions.ContainsKey(AmongUsClient.Instance.HostId) || TheOtherRolesPlugin.Version.CompareTo(playerVersions[AmongUsClient.Instance.HostId].version) != 0)
                    {
                        ...
                        __instance.GameStartText.text = $"<color=#FF0000FF>" + string.Format(ModTranslation.GetString("GameStartText", 5), Math.Round(10 - kickingTimer));
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition + Vector3.up * 5;
                        __instance.GameStartText.transform.localScale = new Vector3(2f, 2f, 1f);
                        __instance.GameStartTextParent.SetActive(true);
                    }
                    ...
                    {
                        __instance.GameStartText.text = $"<color=#FF0000FF>{ModTranslation.GetString("GameStartText", 6)}\n</color>" + message;
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition + Vector3.up * 5;
                        __instance.GameStartText.transform.localScale = new Vector3(2f, 2f, 1f);
                        __instance.GameStartTextParent.SetActive(true);
                    }
