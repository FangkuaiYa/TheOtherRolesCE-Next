    ...
    class MeetingHudPatch
    {
        ...
        class MeetingHudVotingCompletedPatch
        {
            static void Postfix(MeetingHud __instance, [HarmonyArgument(0)] byte[] states, [HarmonyArgument(1)] NetworkedPlayerInfo exiled, [HarmonyArgument(2)] bool tie)
            {
                ...
                if (exiled != null)
                {
                    ...
                    Lawyer.notAckedExiled = (Pursuer.pursuer != null && Pursuer.pursuer.PlayerId == exiled.PlayerId) ||
                ...
                // Snitch
                if (Snitch.snitch != null && !Snitch.needsUpdate && Snitch.snitch.Data.IsDead && Snitch.text != null) UnityEngine.Object.Destroy(Snitch.text);
            }
        }
        ...
        static void swapperOnClick(int i, MeetingHud __instance)
        {
            ...
            else if (selectedCount == 1)
            {
                ...
                else
                {
                    selections[i] = true;
                    renderer.color = Color.yellow;
                    meetingExtraButtonLabel.text = Helpers.cs(Color.yellow, ModTranslation.GetString("Text", 7));
                }
            }
            else if (selectedCount == 2)
            {
                if (selections[i])
                {
                    renderer.color = Color.red;
                    selections[i] = false;
                    meetingExtraButtonLabel.text = Helpers.cs(Color.red, ModTranslation.GetString("Text", 7));
                }
            }
        }
        ...
        static void swapperConfirm(MeetingHud __instance)
        {
            ...
            if (firstPlayer != null && secondPlayer != null)
            {
                ...
                meetingExtraButtonLabel.text = Helpers.cs(Color.green, ModTranslation.GetString("Text", 8));
                Swapper.charges--;
                meetingExtraButtonText.text = $"{ModTranslation.GetString("Text", 9)} {Swapper.charges}";
            }
        }
        ...
        static void mayorToggleVoteTwice(MeetingHud __instance)
        {
            ...
            meetingExtraButtonLabel.text = Helpers.cs(Mayor.color, $"{ModTranslation.GetString("Text", 10)} " + (Mayor.voteTwice ? Helpers.cs(Color.green, ModTranslation.GetString("Text", 11)) : Helpers.cs(Color.red, ModTranslation.GetString("Text", 12))));
        }
        ...
        static void guesserOnClick(int buttonTarget, MeetingHud __instance)
        {
            ...
            foreach (RoleInfo roleInfo in RoleInfo.allRoleInfos)
            {
                ...
                switch (HandleGuesser.isGuesserGm)
                {
                    ...
                    case true when CachedPlayer.LocalPlayer.PlayerControl.Data.Role.IsImpostor && !HandleGuesser.evilGuesserCanGuessSpy && roleInfo.roleId == RoleId.Spy:
        ...
        static void populateButtonsPostfix(MeetingHud __instance)
        {
            ...
            if (addSwapperButtons || addMayorButton)
            {
                ...
                meetingExtraButtonText.text = addSwapperButtons ? $"{ModTranslation.GetString("Text", 9)} {Swapper.charges}" : "";
                ...
                else if (addMayorButton)
                {
                    ...
                    meetingExtraButtonLabel.text = Helpers.cs(Mayor.color, $"{ModTranslation.GetString("Text", 10)} " + (Mayor.voteTwice ? Helpers.cs(Color.green, "On ") : Helpers.cs(Color.red, "Off")));
                }
        ...
        public static void updateMeetingText(MeetingHud __instance)
        {
            ...
            if (numGuesses > 0)
            {
                meetingInfoText = string.Format(ModTranslation.GetString("Text", 13), numGuesses);
            }
            ...
            {
                meetingInfoText = string.Format(ModTranslation.GetString("AdminText", 2), $"{TimeSpan.FromSeconds(Cupid.timeLeft):mm\\:ss}");
            }

            if (CachedPlayer.LocalPlayer.PlayerControl == Doomsayer.doomsayer)
            {
                meetingInfoText = string.Format(ModTranslation.GetString("Text", 14), Doomsayer.killToWin - Doomsayer.killedToWin);
            }
        ...
        class StartMeetingPatch
        {
            public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo meetingTarget)
            {
                ...
                if (Portalmaker.portalmaker != null && (CachedPlayer.LocalPlayer.PlayerControl == Portalmaker.portalmaker || Helpers.shouldShowGhostInfo()) && !Portalmaker.portalmaker.Data.IsDead)
                {
                    ...
                    {
                        string msg = $"{ModTranslation.GetString("Text", 15)}\n";
                        foreach (var entry in Portal.teleportedPlayers)
                        {
                            ...
                            msg += Portalmaker.logShowsTime ? string.Format($"{ModTranslation.GetString("Text", 16)}: ", (int)timeBeforeMeeting) : "";
                            msg = msg + string.Format(ModTranslation.GetString("Text", 17), entry.name);
                        }
                ...
                if (Trapper.trapper != null && (CachedPlayer.LocalPlayer.PlayerControl == Trapper.trapper || Helpers.shouldShowGhostInfo()) && !Trapper.trapper.Data.IsDead)
                {
                    if (Trap.traps.Any(x => x.revealed))
                        FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(Trapper.trapper, ModTranslation.GetString("Text", 18));
                    foreach (Trap trap in Trap.traps)
                    {
                        if (!trap.revealed) continue;
                        string message = string.Format(ModTranslation.GetString("Text", 19), trap.instanceId);
                        ...
                        foreach (PlayerControl p in trap.trappedPlayer)
                        {
                            ...
                            {
                                if (Helpers.isNeutral(p) || p.Data.Role.IsImpostor) message += $"{ModTranslation.GetString("Text", 20)} \n";
                                else message += $"{ModTranslation.GetString("Text", 21)} \n";
                            }
            ...
            public class ShowHost
            {
                ...
                public static void Postfix(MeetingHud __instance)
                {
                    ...
                    if (host != null)
                    {
                        ...
                        Text.text = string.Format(ModTranslation.GetString("Text", 22), host.PlayerName);
                    }
                }
            }
        }
        ...
        public class BlackmailerPlayerQuickChatMenu
        {
            public static bool Prefix(QuickChatMenu __instance)
            {
                if (Blackmailer.blackmailer != null && Blackmailer.blackmailed != null && PlayerControl.LocalPlayer == Blackmailer.blackmailed)
                {
                    return false;
                }
                return true;
        ...
        public class MeetingHudStart
        {
            ...
            public static void Postfix(MeetingHud __instance)
            {
                ...
                if (Blackmailer.blackmailed.Data.PlayerId == CachedPlayer.LocalPlayer.PlayerId && !Blackmailer.blackmailed.Data.IsDead && Blackmailer.blackmailed == CachedPlayer.LocalPlayer.PlayerControl)
                {
