...
namespace TheOtherRoles.Patches
{
    ...
    class CreateOptionsPickerPatch
    {
        ...
        public static bool Prefix(CreateOptionsPicker __instance, ref GameModes mode)
        {
            ...
            {
                __instance.GameModeText.text = ModTranslation.GetString("CreateGameModeText", 1);
                MapOptionsTor.gameMode = CustomGamemodes.Guesser;
            }
            else if (gm == CustomGamemodes.HideNSeek)
            {
                __instance.GameModeText.text = ModTranslation.GetString("CreateGameModeText", 2);
                MapOptionsTor.gameMode = CustomGamemodes.HideNSeek;
            }
            else if (gm == CustomGamemodes.PropHunt)
            {
                __instance.GameModeText.text = ModTranslation.GetString("CreateGameModeText", 3);
        ...
        public static bool Prefix(GameModeMenu __instance)
        {
            ...
            for (int i = 0; i <= 5; i++)
            {
                ...
                if (entry != GameModes.None)
                {
                    ...
                    {
                        chatLanguageButton.Text.text = i == 3 ? ModTranslation.GetString("CreateGameModeText", 1) : ModTranslation.GetString("CreateGameModeText", 2);
                        if (i == 5)
                            chatLanguageButton.Text.text = ModTranslation.GetString("CreateGameModeText", 3);
                    }
