	...
	class HudManagerUpdatePatch
	{
		...
		static void setNameColors()
		{
			...
			if (Sidekick.sidekick != null && Sidekick.sidekick == localPlayer)
			{
				...
				{
					setPlayerNameColor(Jackal.jackal, Jackal.jackal == Swooper.swooper ? Swooper.color : Jackal.color);
				}
			}
			...
			{
				for (var index = 0; index < Arsonist.dousedPlayers.Count; index++)
				{
					var p = Arsonist.dousedPlayers[index];
		...
		static void setNameTags()
		{
			...
			if (CachedPlayer.LocalPlayer != null && CachedPlayer.LocalPlayer.Data.Role.IsImpostor)
			{
				foreach (PlayerControl player in CachedPlayer.AllPlayers)
					if (Godfather.godfather != null && Godfather.godfather == player)
						player.cosmetics.nameText.text = player.Data.PlayerName + $" {ModTranslation.GetString("Text", 33)}";
					else if (Mafioso.mafioso != null && Mafioso.mafioso == player)
						player.cosmetics.nameText.text = player.Data.PlayerName + $" {ModTranslation.GetString("Text", 34)}";
					else if (Janitor.janitor != null && Janitor.janitor == player)
						player.cosmetics.nameText.text = player.Data.PlayerName + $" {ModTranslation.GetString("Text", 35)}";
				if (MeetingHud.Instance != null)
					foreach (PlayerVoteArea player in MeetingHud.Instance.playerStates)
						if (Godfather.godfather != null && Godfather.godfather.PlayerId == player.TargetPlayerId)
							player.NameText.text = Godfather.godfather.Data.PlayerName + $" {ModTranslation.GetString("Text", 33)}";
						else if (Mafioso.mafioso != null && Mafioso.mafioso.PlayerId == player.TargetPlayerId)
							player.NameText.text = Mafioso.mafioso.Data.PlayerName + $" {ModTranslation.GetString("Text", 34)}";
						else if (Janitor.janitor != null && Janitor.janitor.PlayerId == player.TargetPlayerId)
							player.NameText.text = Janitor.janitor.Data.PlayerName + $" {ModTranslation.GetString("Text", 35)}";
			}
			...
			if (CachedPlayer.LocalPlayer != null && MeetingHud.Instance != null && MapOptionsTor.showLighterDarker)
			{
				foreach (PlayerVoteArea player in MeetingHud.Instance.playerStates)
				{
					...
					if (target != null) player.NameText.text += $" ({(Helpers.isLighterColor(target) ? ModTranslation.GetString("Text", 36) : ModTranslation.GetString("Text", 37))})";
				}
			}
