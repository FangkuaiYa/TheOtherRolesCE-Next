...
namespace TheOtherRoles
{
	...
	public static class TheOtherRoles
	{
		...
		public static class Sheriff
		{
			...
			{
				if (buttonSprite) return buttonSprite;
				buttonSprite = CustomMain.customZips.SheriffKillButton;
				return buttonSprite;
			}
		...
		public static class Deputy
		{
			...
			public static Sprite getHandcuffedButtonSprite()
			{
				...
				handcuffedSprite = DataManager.Settings.Language.CurrentLanguage == SupportedLangs.Irish ? CustomMain.customZips.DeputyHandcuffedCN : CustomMain.customZips.DeputyHandcuffedEN;
	...
	public static class Medium
	{
		...
		public static string getInfo(PlayerControl target, PlayerControl killer)
		{
			...
			if (infos.Count > 0)
			{
				...
				switch (selectedInfo)
				{
					case SpecialMediumInfo.SheriffSuicide:
						msg = ModTranslation.GetString("SpecialMediumInfo", 1);
						break;
					case SpecialMediumInfo.WarlockSuicide:
						msg = ModTranslation.GetString("SpecialMediumInfo", 2);
						break;
					case SpecialMediumInfo.ThiefSuicide:
						msg = ModTranslation.GetString("SpecialMediumInfo", 3);
						break;
					case SpecialMediumInfo.ActiveLoverDies:
						msg = ModTranslation.GetString("SpecialMediumInfo", 4);
						break;
					case SpecialMediumInfo.PassiveLoverSuicide:
						msg = ModTranslation.GetString("SpecialMediumInfo", 5);
						break;
					case SpecialMediumInfo.LawyerKilledByClient:
						msg = ModTranslation.GetString("SpecialMediumInfo", 6);
						break;
					case SpecialMediumInfo.JackalKillsSidekick:
						msg = ModTranslation.GetString("SpecialMediumInfo", 7);
						break;
					case SpecialMediumInfo.ImpostorTeamkill:
						msg = ModTranslation.GetString("SpecialMediumInfo", 8);
						break;
					case SpecialMediumInfo.BodyCleaned:
						msg = ModTranslation.GetString("SpecialMediumInfo", 9);
						break;
				}
			}
			else
			{
				...
				string typeOfColor = Helpers.isLighterColor(Medium.target.killerIfExisting) ? ModTranslation.GetString("Text", 30) : ModTranslation.GetString("Text", 31);
				...
				if (randomNumber == 0)
				{
					if (!roleString.Contains(RoleInfo.impostor.name) && !roleString.Contains(RoleInfo.crewmate.name))
						msg = string.Format(ModTranslation.GetString("SpecialMediumInfo", 10), roleString);
					else
						msg = string.Format(ModTranslation.GetString("SpecialMediumInfo", 11), roleString);
				}
				else if (randomNumber == 1) msg = string.Format(ModTranslation.GetString("SpecialMediumInfo", 12), typeOfColor);
				else if (randomNumber == 2) msg = string.Format(ModTranslation.GetString("SpecialMediumInfo", 13), Math.Round(timeSinceDeath / 1000));
				else msg = string.Format(ModTranslation.GetString("SpecialMediumInfo", 14), RoleInfo.GetRolesString(Medium.target.killerIfExisting, false, false, true));
			}
			...
			if (rnd.NextDouble() < chanceAdditionalInfo)
			{
				...
				switch (rnd.Next(3))
				{
					case 0:
						...
						condition = DataManager.Settings.Language.CurrentLanguage == SupportedLangs.SChinese ? ModTranslation.GetString("ChineseText", 1) : "killer" + (count == 1 ? "" : "s");
						break;
					case 1:
						count = alivePlayersList.Where(Helpers.roleCanUseVents).Count();
						condition = DataManager.Settings.Language.CurrentLanguage == SupportedLangs.SChinese ? ModTranslation.GetString("ChineseText", 2) : "player" + (count == 1 ? "" : "s") + " who can use vents";
						break;
					case 2:
						count = alivePlayersList.Where(pc => Helpers.isNeutral(pc) && pc != Jackal.jackal && pc != Sidekick.sidekick && pc != Thief.thief).Count();
						condition = DataManager.Settings.Language.CurrentLanguage == SupportedLangs.SChinese ? ModTranslation.GetString("ChineseText", 3) : "player" + (count == 1 ? "" : "s") + " who " + (count == 1 ? "is" : "are") + " neutral but cannot kill";
						break;
					case 3:
						//count = alivePlayersList.Where(pc =>
						break;
				}
				msg += DataManager.Settings.Language.CurrentLanguage == SupportedLangs.SChinese ? string.Format(ModTranslation.GetString("ChineseText", 4), condition) : $"\nWhen you asked, {count} " + condition + (count == 1 ? " was" : " were") + " still alive";
			}
			return Medium.target.player.Data.PlayerName + ModTranslation.GetString("SpecialMediumInfo", 15) + msg;
		}
	}
	...
	public static class Doomsayer
	{
		...
		public static string GetInfo(PlayerControl target)
		{
			try
			{
				...
				if (allRoleInfo.Count < formationNum + 2) return string.Format(ModTranslation.GetString("Game-Doomsayer", 1), formationNum + 2);
				...
				message.AppendLine(string.Format(ModTranslation.GetString("Game-Doomsayer", 2), target.Data.PlayerName));
	...
	{
		public static List<PlayerControl> survivor = new List<PlayerControl>();
		public static PlayerControl target;
		public static List<PlayerControl> blankedList = new List<PlayerControl>();
		...
		{
			if (buttonSprite) return buttonSprite;
			buttonSprite = CustomMain.customZips.VestButton;
			return buttonSprite;
		}
		public static void clearAndReload()
		{
			...
			vestNumber = (int)CustomOptionHolder.survivorVestNumber.getFloat();
			...
			blanksNumber = (int)CustomOptionHolder.survivorBlanksNumber.getFloat();
	...
	public static class Juggernaut
	{
		...
		public static PlayerControl currentTarget;
		public static float cooldown = 30f;
		public static float cooldownReductionPerKill = 5f;
		public static bool canVent = false;
		public static int JuggKills { get; set; }

		public static void clearAndReload()
		{
			juggernaut = null;
			currentTarget = null;
			cooldown = CustomOptionHolder.juggernautKillCooldown.getFloat();
			cooldownReductionPerKill = CustomOptionHolder.juggernautReductionPerKill.getFloat();
	...
	public static class PlagueDoctor
	{
		...
		public static Sprite getSyringeIcon()
		{
			...
			plagueDoctorIcon = CustomMain.customZips.InfectButton;
	...
	public static class Cupid
	{
		...
		public static Sprite getArrowSprite()
		{
			...
			arrowSprite = CustomMain.customZips.CupidButton;
			return arrowSprite;
		}
